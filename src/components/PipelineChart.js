...
export function PipelineChart({ deals, onTotalExpectedRevenueChange }) {
  ...
  const [selectedSalesperson, setSelectedSalesperson] = useState('全員');

  const salespeople = ['全員', ...new Set(deals.map(deal => deal['営業担当者名']))];
  ...
  useEffect(() => {
    ...
    const filteredDeals = selectedSalesperson === '全員'
      ? deals
      : deals.filter(deal => deal['営業担当者名'] === selectedSalesperson);
    ...
    totalChartInstance.current = new Chart(totalCtx, {
      ...
      data: {
        labels: ['合計'],
        datasets: phaseLabels.map((phase, index) => ({
          label: phase,
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div style={{ marginBottom: '16px' }}>
        <label htmlFor="salesperson-select" style={{ marginRight: '8px' }}>営業担当者:</label>
        <select
